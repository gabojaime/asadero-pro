...
export function SignUpForm({ className, ...props }: React.ComponentPropsWithoutRef<"div">) {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.auth.signUp({ email, password, options: { emailRedirectTo: `${window.location.origin}/protected` } });
      ...
      <Card className="border-border/80 bg-card/95 shadow-xl shadow-primary/5">
        ...
        <CardContent className="p-7 pt-0">
          <form onSubmit={handleSignUp} className="flex flex-col gap-5">
            ...
            <p className="text-center text-sm text-muted-foreground">¿Ya tienes cuenta? <Link href="/auth/login" className="font-medium text-foreground underline underline-offset-4 hover:text-primary">Inicia sesión</Link></p>
          </form>
        </CardContent>
      </Card>
