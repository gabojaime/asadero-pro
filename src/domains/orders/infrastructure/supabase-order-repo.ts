...
function mapOrderRow(row: OrderWithRelations): Order {
  return {
    ...
    readyByAt: row.ready_by_at ? new Date(row.ready_by_at) : null,
    ...
    async insertOrder(params: InsertOrderParams) {
      ...
      const { data: orderId, error: rpcError } = await supabase.rpc(
        "create_order_with_items",
        {
          ...
          p_fulfillment_timing: params.fulfillmentTiming,
          p_ready_by_at: params.readyByAt?.toISOString() ?? null,
          p_customer_first_name: params.customerFirstName,
          p_customer_last_name: params.customerLastName,
